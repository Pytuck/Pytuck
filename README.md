...
> 运行 `uv run python tests/benchmark/benchmark_encryption.py` 可在您的环境中进行加密专项性能测试。
...
以下结果来自当前仓库内置的 benchmark 脚本，而不是历史版本的静态表格。

### 测试环境

- **系统**: Linux 6.18.7-76061807-generic
- **Python**: 3.12.3
- **测试数据量**: 100,000 条记录
- **模式**: 扩展测试（包含索引对比、范围查询、批量读取、懒加载查询）
- **命令**: `uv run python tests/benchmark/benchmark.py -n 100000 --extended --output-json /tmp/pytuck-benchmark-final.json`
...
- [x] **DuckDB 引擎** - 原生 DuckDB 后端，支持多 schema、原生 SQL、注释与服务端分页
