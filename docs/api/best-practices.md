...
> | 限制 | 说明 |
> |------|------|
> | **数据规模** | 这里的“万级 / 十万级”主要指**单张热点表**的记录数。万级记录以内体验最佳；当单表接近或超过 10 万条时，优先使用 DuckDB / SQLite 引擎（原生 SQL 模式），百万级以上不建议使用本库 |
...
>
> 注：如果多张表的总记录量已经接近或超过这个量级，整体 I/O、索引维护和加载时间同样会明显上升，也应优先评估 DuckDB / SQLite 或外部数据库方案。
>
> ### 何时应该选择其他方案
>
> | 你的需求 | 推荐方案 |
...
> ### Pytuck 的最佳使用场景
>
> - **受限 CPython 环境**（如 Ren'Py 中无法可靠安装第三方包或 C 扩展的场景）
> - **桌面应用 / 小工具**的本地数据存储
> - **原型开发 / 快速验证**（零配置、零 SQL）
> - **配置管理 / 小规模数据持久化 / 游戏存档**
...
多表数据库只修改部分表后执行 `flush()` 时，Pytuck 会直接复用未修改 lazy 表的 PTK7 编码块；未修改表不会仅为保存而全部加载到内存。文件仍通过临时文件和原子替换完成整体更新，PTK7 格式保持不变。
