...
- **Binary 引擎加密支持**：三级加密/混淆功能，纯 Python 实现，零外部依赖
  ...
  - 使用示例：
    ...
    # 创建加密数据库
    opts = PytuckBackendOptions(encryption='high', password='mypassword')
    db = Storage('data.db', engine='pytuck', backend_options=opts)
    ```
...
- **Single-File Engine v4 Format**: New storage format optimized for large dataset performance
...
- **Single-File Engine Encryption Support**: Three-tier encryption/obfuscation, pure Python implementation, zero external dependencies
...
- **Single-File Engine Performance Improvements**
