...
- `DuckDB` 的 reopen 和查询能力依旧很强，但当前 ORM / Session 逐条写入路径在插入 benchmark 上表现很差：`277.18s`。
- 这也是 `TODO.md` 中仍保留“duckdb 引擎优化”任务的直接原因。
- 在当前实现下，更推荐把 `duckdb` 用于分析查询、已有 DuckDB 文件接入和原生 SQL 场景，而不是高频 ORM 批量写入。
