...
## PTK7 多表增量 flush 补充 benchmark

> 测试时间：2026-07-14
>
> 测试环境：macOS 26.5 / Python 3.13.11
>
> 数据规模：4 张表，每表 5000 条记录；重开后只修改第一张表的一条记录；重复 5 次取中位数

| 实现 | flush 中位耗时 | 峰值内存中位数 | 未修改表 materialize 行数 |
|------|----------------|------------------|-----------------------------|
| 优化前（`c7c61d8`） | 1.280s | 20.24MB | 5000 |
| 当前实现 | 0.474s | 11.87MB | 0 |

- 未修改 lazy 表直接复用 PTK7 编码块，flush 约提升 `2.70x`
- 峰值内存约下降 `41%`
- 文件大小保持一致，PTK7 格式和原子替换策略不变
...
- `主键查询` 与 `索引查询` 都维持在毫秒级，适合需要完整类型保留、零依赖单文件存储，以及更快打开 / 重开体验的场景。
- 当前默认单文件路径的主目标是格式长期稳定、实现收敛和读路径体验；不应把这张表理解为 `Pytuck` 在所有 benchmark 维度都全面领先。
