"""
Pytuck Session + Statement API 完整示例
...
def main() -> None:
    ...
    # 另一种方式：session.add() + flush()
    eve = Student(name='Eve', age=20, class_id=class_a_id)
    session.add(eve)
    session.flush()
    session.commit()
    print(f"   ✓ session.add + flush: Eve (ID: {eve.id})")
    ...
    print("总结")
    print("=" * 60)
    print("Session + Statement API 特点:")
    ...
    print("\n推荐：所有新项目使用 Session + Statement API！")
