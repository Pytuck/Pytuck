"""
PTK5 legacy 读取与运行时实现。

该模块承接从 backend_binary.py 抽离出的 PTK5/WAL/legacy crypto 逻辑，
用于 legacy 识别、读取和迁移。
"""
...
def probe_ptk5(file_path: Union[str, Path]) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """探测给定文件是否为 PTK5 legacy 文件。"""
    try:
        ...
        with open(path, 'rb') as handle:
            magic = handle.read(4)
            if magic != BinaryBackend.MAGIC_V5:
                return False, None

        if file_size < BinaryBackend.DUAL_HEADER_SIZE:
            return False, {'error': 'file_too_small_for_checkpoint'}
        ...
        return False, {'error': f'probe_exception: {str(exc)}'}


def load_ptk5_tables(
    file_path: Union[str, Path],
    options: Optional[BinaryBackendOptions] = None,
) -> Dict[str, 'Table']:
    """读取 PTK5 legacy 文件中的表数据。

    为了防止在运行时被其他模块（如 backend_binary）通过 monkeypatch 修改
    BinaryBackend.__init__ 或 BinaryBackend.load，我们在模块加载时保存
    原始的构造器和 load 方法引用，并在此处使用这些稳定引用来创建和
    调用后端实例，从而确保迁移路径不受全局 monkeypatch 的影响。
    """
    backend_options = options or BinaryBackendOptions()

    # 使用 object.__new__ 创建实例并调用模块级保存的原始引用，以绕过可能的 monkeypatch
    try:
        _orig_init = globals().get('_LEGACY_BINARY_BACKEND_INIT', BinaryBackend.__init__)
        _orig_load = globals().get('_LEGACY_BINARY_BACKEND_LOAD', BinaryBackend.load)
    except Exception:
        _orig_init = BinaryBackend.__init__
        _orig_load = BinaryBackend.load

    backend = object.__new__(BinaryBackend)
    # 调用保存的原始构造器初始化实例
    _orig_init(backend, file_path, backend_options)

    # 调用保存的原始 load 方法并返回结果
    return _orig_load(backend)

# 模块级的稳定引用，避免污染 BinaryBackend 类命名空间
try:
    _LEGACY_BINARY_BACKEND_INIT = BinaryBackend.__init__
    _LEGACY_BINARY_BACKEND_LOAD = BinaryBackend.load
except Exception:
    _LEGACY_BINARY_BACKEND_INIT = BinaryBackend.__init__
    _LEGACY_BINARY_BACKEND_LOAD = BinaryBackend.load
